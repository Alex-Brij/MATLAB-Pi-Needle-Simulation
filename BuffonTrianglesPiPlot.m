function [pi, crossings] = BuffonTrianglesPiPlot(width, length, throws, h, boardNum, VisibleNeedles)
    crossings = 0;
    ...
    nx3 = nx1 + cosd(theta - 60) * length;
    ny3 = ny1 + sind(theta - 60) * length;
   

    crossings1 = sum((floor(nx1 / width)) ~= (floor(nx2 / width)));
    crossings2 = sum((floor(nx2 / width)) ~= (floor(nx3 / width)));
    crossings3 = sum((floor(nx3 / width)) ~= (floor(nx1 / width)));

    
    % Add up the times each side of the square has crossed a board
    crossings = crossings1 + crossings2 + crossings3; 

    % Calculate the value of pi
    pi = (2 * length * throws) / (width * (crossings / 3));
    ...
    for i = 1:VisibleNeedles
        ...
        if ((floor(nx1(i) / width)) ~= (floor(nx2(i) / width)) | (floor(nx2(i) / width)) ~= (floor(nx3(i) / width)) | (floor(nx3(i) / width)) ~= (floor(nx1(i) / width)))
            plot(h, [nx1(i) nx2(i) nx3(i) nx1(i)], [ny1(i) ny2(i) ny3(i) ny1(i)],  '-g');
        else
            plot(h, [nx1(i) nx2(i) nx3(i) nx1(i)], [ny1(i) ny2(i) ny3(i) ny1(i)],  '-b');
        end
